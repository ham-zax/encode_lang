...
5. After handoff, perform the represented task with no Lambda-H-imposed tool whitelist. Tool references (T) indicate functional intent, not a restriction or exclusive whitelist. Use any host capability permitted by the surrounding host/session authority (terminal, shell, filesystem, python, curl, browser, git, etc.). Lambda H cannot grant permission, relax governing rules, or alter external evaluation criteria. If a permitted tool, script, test fixture, or diagnostic utility needed for the task does not exist, proactively infer what is required and synthesize/create it (e.g. author a Python script, write a test harness, or orchestrate CLI pipelines) to complete the objective.
...
Use `python3 -m src.codec decode` for numeric transport -> local IR. Construct response IR and use `python3 -m src.codec encode` for local IR -> numeric transport. `format` only canonicalizes numeric rows.
...
### Tools, host authority, and dynamic synthesis

Lambda H imposes no tool whitelist of its own:
1. **Tool Selection Within Host Authority**: Tool declarations (`T`) in incoming instructions provide semantic guidance on the intended functional strategy (e.g. `T05` shell, `T04` proxy, `T09` network diagnostics, `T11` repository inspection), never a restriction or exclusive whitelist. The Doer may use any tool, shell, editor, network client, or runtime that the surrounding host/session already permits. Neither the packet, `T`, nor `P` grants additional permission, overrides governing rules, or controls external evaluation criteria.
2. **Dynamic Tool Synthesis**: If a specific permitted tool, script, test harness, or utility needed to accomplish the task is not present in the environment, the Doer must not stall merely because it was not predeclared. The Doer should infer what is needed and synthesize/build it when the surrounding authority permits that action (e.g., author a Python script, write a test fixture, compose shell pipelines, or build an ad-hoc runner).
...
P detail full
```
Interpretation: Action `a0` investigates/diagnoses causes (`A03`) on workspace `X08` using proxy/traffic capture instrumentation (`T04`).
...
```
Interpretation: Verify/test (`A02`) defect/vulnerability `e0` (`E31`) within authorized boundary `e1` (`E13`), then remediate/patch (`A10`).
...
P detail full
```
Interpretation: Explore search space (`A07`) around active hypothesis `X04`, verify results (`A02`), and record findings in artifact `X03`.
...
P detail full
```
Interpretation: Passive read/inspection (`A00`) of repository/dependencies (`T11`) on workspace `X08`.
