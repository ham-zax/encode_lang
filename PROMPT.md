...
Repo-aware hosts may establish local ambient context for X02 goal, X03 artifact, X06 plan, X07 blocker, X08 workspace/environment/repository, and X09 output/result target. `context 0` is reserved for receiver-current ambient state. For X08, bind the workspace already active at packet receipt: prefer a host/IDE workspace root, otherwise the current process/tool working directory, optionally normalized only to its enclosing Git worktree root. Never discover X08 by scanning `/home`, `/`, sibling repositories, caches, or unrelated worktrees. Freeze that binding for the request. Nonzero contexts remain strict and require an exact same-namespace host binding. Deictic encoding such as “this repo” should use `context 0` + X08 rather than inventing a generic E node.
