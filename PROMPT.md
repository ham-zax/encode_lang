...
Repo-aware hosts may establish local ambient context for X00 current conversational/task subject, X02 goal, X03 artifact, X06 plan, X07 blocker, X08 workspace/environment/repository, and X09 output/result target. `context 0` is reserved for receiver-current ambient state. X00 may inherit exactly one already-established unambiguous conversational subject at packet receipt; multiple/no subjects remain missing, and X00 must not be inferred from cwd/X08 alone. For X08, bind the workspace already active at packet receipt: prefer a host/IDE workspace root, otherwise the current process/tool working directory, optionally normalized only to its enclosing Git worktree root. Never discover X08 by scanning `/home`, `/`, sibling repositories, caches, or unrelated worktrees. Freeze context-0 bindings for the request. Nonzero contexts remain strict and require an exact same-namespace host binding. Encode deictics by their actual role: “this repo” -> X08, while a genuinely current project/topic subject may use X00.
