...
Use `context 0` only when the packet intentionally refers to the receiver's current host/session state. In that namespace, X00/X02/X03/X06/X07/X08/X09 may be grounded locally from already-established state. X00 may use exactly one unambiguous conversational/task subject present before packet receipt; it must not be inferred from cwd or repository identity alone. For X08, use the host/IDE workspace root or current process/tool working directory (optionally normalized only to its enclosing Git worktree root); never search other directories or repositories to select an identity. Freeze selected context-0 identities for the request. Use a fresh nonzero, non-identifying context namespace when identity must remain stable across sessions.

Lambda H/2.2 does not ship a readable context-sidecar or handoff command. The numeric wire may bind only genuine numeric, boolean, or null values. `src.context.HostContext` may hold richer endpoint-local objects or textual identities because it never serializes them. Nonzero contexts require an exact packet/host namespace match; a receiver must not reinterpret nonzero X00/X08 from its current conversation or open repository. If a required binding is unavailable under these rules, the protocol returns `need`; it does not hide the text inside numbers.
