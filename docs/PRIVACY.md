...
Use `context 0` only when the packet intentionally refers to the receiver's current host/session state. In that namespace, directly observable unambiguous ambient facts already active at packet receipt may ground X02/X03/X06/X07/X08/X09 locally. For X08, use the host/IDE workspace root or current process/tool working directory (optionally normalized only to its enclosing Git worktree root); never search other directories or repositories to select an identity. Freeze the selected context-0 identity for the request. Use a fresh nonzero, non-identifying context namespace when identity must remain stable across sessions.

Lambda H/2.2 does not ship a readable context-sidecar or handoff command. The numeric wire may bind only genuine numeric, boolean, or null values. `src.context.HostContext` may hold richer endpoint-local objects or textual identities because it never serializes them. Nonzero contexts require an exact packet/host namespace match; a receiver must not reinterpret a nonzero-context X08 as some other repository merely because it has one open. If a required binding is unavailable under these rules, the protocol returns `need`; it does not hide the text inside numbers.
