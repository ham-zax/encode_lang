...
Use `context 0` only when the packet intentionally refers to the receiver's current host/session state. In that namespace, X00/X02/X03/X06/X07/X08/X09 may be grounded locally from already-established state. Freeze selected context-0 identities for the request. Use a fresh nonzero, non-identifying context namespace when identity must remain stable across sessions.

Lambda H/2.2 does not ship a readable context-sidecar or handoff command. The numeric wire may bind only genuine numeric, boolean, or null values. `src.context.HostContext` may hold richer endpoint-local objects or textual identities because it never serializes them. Resolution precedence is packet-inline X value, then exact host/session binding, then the receiver's best available interpretation; the wire never hides text inside numbers. If exact identity matters, use a packet-inline X value or an exact host/session binding.
