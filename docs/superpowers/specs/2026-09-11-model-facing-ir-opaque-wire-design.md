...
X references are namespace-scoped handles. `X00`, `X02`, `X03`, `X06`, `X07`, `X08`, and `X09` are context-0 ambient-capable conventions for current conversational/task subject, active goal, artifact, plan, blocker, workspace/environment/repository, and output/result target. Select by semantic role: X00 and X08 remain distinct. Context-0 X00 may use one already-established unambiguous conversational subject; nonzero contexts require explicit same-namespace host grounding. See [Agent-native ambient context](2026-09-11-agent-native-ambient-context-design.md).

Resolution uses packet-inline values first, then exact matching host/session bindings; context-0 X00 may then use one already-established unambiguous conversational subject, otherwise the reference is missing. For X08, use the host/IDE workspace root or current cwd/enclosing Git worktree root and never scan elsewhere to discover a candidate repository. Freeze context-0 bindings for the request. The Encoder may classify required X refs locally and surface accidental unresolved refs as `UNBOUND_REQUIRED_X`; this is not a transport control. Host-local values may be richer endpoint objects/text because they never become Lambda H transport. Receiver-current state must never satisfy a different nonzero namespace. Bind frames still carry only genuine numeric, boolean, or null values.
