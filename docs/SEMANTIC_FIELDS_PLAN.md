...
Implement squared-exponential component compatibility and combine alternatives with a relative-weight max-envelope rather than summing overlapping lobes. Scores are relative geometric compatibility, not truth/confidence/word identity. Widths may differ on either side of each axis. Candidate ranking returns all scores with caller-supplied minimum and margin thresholds, never a fabricated best-word decode. Add a codec score command using a declared node and a numerical candidate file. Demonstrate center, directional tails, distant candidates and preserved alternatives with synthetic data.

**Acceptance:** A single-component center scores 1; narrower sides decay faster; equal-weight alternative centers retain their own peaks and overlap cannot create a stronger synthetic midpoint; components and relation direction remain distinct; no candidate dictionary or network request is involved.

## Task 3: Receiver and contract closure

**Files:** prompt/BOOTSTRAP.md; prompt/ENCODER.md; SPEC.md; README.md; PROMPT.md; MIGRATION.md; examples/; calibration/probes.json; calibration/README.md; calibration/RESULTS.md; src/calibration.py; docs/PRIVACY.md.

Use separate standalone role prompts: the receiver bootstrap teaches a deterministic unpack -> graph -> field -> constraint -> action path, while the encoder bootstrap teaches source -> graph -> numeric-wire construction. Both retain field-first semantics and optional Python arithmetic without prescribing hidden thought language. Show numeric fields before exact-word examples. Retain old experiment identities and mark them historical. The behavior recorder remains receiver-only, distinguishes optional geometric computation from task tools, and must not treat a permitted decoder call as failure by itself. Synchronize role documentation with the unchanged wire contract.
...
## Implementation observations — updated 2026-09-11
...
- Python decoding and field arithmetic are allowed. The receiver bootstrap defaults protocol replies to numeric packets unless prose is requested and now gives an explicit mechanical decode path; the separate encoder bootstrap constructs packets without executing their source tasks. No hidden-reasoning-language claim is made.
- Six active Python modules passed syntax inspection. The exported graph schema equals its owner, both role prompts share the same tag/enum tables and anchor block, those anchor sets agree with the basis, 16 corpus packets and 4 wire samples round-trip, and 15 numeric documentation examples parse. Current local documentation links resolve.
